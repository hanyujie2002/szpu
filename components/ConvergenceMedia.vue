<template>
  <section
    class="home4-bg relative z-10 bg-cover bg-fixed bg-top bg-no-repeat pt-[59.5px]"
  >
    <div
      class="absolute bottom-0 left-0 right-0 -z-10 h-[340px] bg-white md:h-[142.792px]"
    />
    <div
      class="px-[15px] md:px-[59.5px] lg:mx-auto lg:w-[1200px] lg:max-w-[94%] lg:px-0"
    >
      <div
        class="z-10 mb-[46.75px] flex items-center justify-between !duration-1000"
        data-aos="fade-up"
      >
        <NuxtLink to="kxyj/xsky">
          <div
            class="relative pl-[17px] before:absolute before:bottom-0 before:left-0 before:top-0 before:w-[3px] before:bg-white before:content-['']"
          >
            <div
              class="uppercase text-white"
              style="font-size: 13.6px; line-height: 13.6px"
            >
              convergence media
            </div>
            <div
              class="mt-[8.5px] font-semibold text-white"
              style="font-size: 28.9px; line-height: 28.9px"
            >
              深职融媒
            </div>
          </div>
        </NuxtLink>
        <div>
          <ul class="flex">
            <li class="group relative z-10 ml-[17px] list-item cursor-pointer">
              <div
                class="flex h-[44.1979px] w-[44.1979px] items-center rounded-full bg-white/20 duration-500 before:absolute before:bottom-0 before:left-0 before:right-0 before:top-0 before:scale-0 before:rounded-full before:border-[1px] before:border-white/70 before:duration-500 before:content-[''] group-hover:bg-white/50 before:group-hover:scale-125"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico1.png"
                  class="h-full w-full drop-shadow-custom group-hover:drop-shadow-custom-1"
                />
              </div>
              <div
                class="before-[''] invisible absolute bottom-[-140.927px] left-[22.094px] right-[-16.146px] top-[61.875px] z-20 ml-[-85px] w-[123.25px] translate-y-[8.5px] rounded-[8.5px] bg-white p-[12.75px] opacity-0 transition-all duration-500 before:absolute before:bottom-[118.146px] before:left-[80.76px] before:right-[32.3px] before:top-[-5.1px] before:h-[10.1979px] before:w-[10.1979px] before:rotate-45 before:bg-white before:transition-all before:duration-500 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  class="h-full w-full"
                  src="/images/weibo-szpt.png"
                />
              </div>
            </li>

            <li class="group relative z-10 ml-[17px] list-item cursor-pointer">
              <div
                class="flex h-[44.1979px] w-[44.1979px] items-center rounded-full bg-white/20 transition-all duration-500 before:absolute before:bottom-0 before:left-0 before:right-0 before:top-0 before:scale-0 before:rounded-full before:border-[1px] before:border-white/70 before:transition-all before:duration-500 before:content-[''] group-hover:bg-white/50 before:group-hover:scale-125"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="h-full w-full drop-shadow-custom group-hover:drop-shadow-custom-1"
                />
              </div>
              <div
                class="before-[''] invisible absolute bottom-[-140.927px] left-[22.094px] right-[-16.146px] top-[61.875px] z-10 ml-[-85px] w-[123.25px] translate-y-[8.5px] rounded-[8.5px] bg-white p-[12.75px] opacity-0 transition-all duration-500 before:absolute before:bottom-[118.146px] before:left-[80.76px] before:right-[32.3px] before:top-[-5.1px] before:h-[10.1979px] before:w-[10.1979px] before:rotate-45 before:bg-white before:transition-all before:duration-500 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  class="h-full w-full"
                  src="/images/new_ewm.jpg"
                />
              </div>
            </li>

            <li class="group relative z-10 ml-[17px] list-item cursor-pointer">
              <div
                class="flex h-[44.1979px] w-[44.1979px] items-center rounded-full bg-white/20 duration-500 before:absolute before:bottom-0 before:left-0 before:right-0 before:top-0 before:scale-0 before:rounded-full before:border-[1px] before:border-white/70 before:duration-500 before:content-[''] group-hover:bg-white/50 before:group-hover:scale-125"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico3.png"
                  class="h-full w-full drop-shadow-custom group-hover:drop-shadow-custom-1"
                />
              </div>
              <div
                class="before-[''] invisible absolute bottom-[-140.927px] left-[22.094px] right-[-16.146px] top-[61.875px] z-10 ml-[-85px] w-[123.25px] translate-y-[8.5px] rounded-[8.5px] bg-white p-[12.75px] opacity-0 transition-all duration-500 before:absolute before:bottom-[118.146px] before:left-[80.76px] before:right-[32.3px] before:top-[-5.1px] before:h-[10.1979px] before:w-[10.1979px] before:rotate-45 before:bg-white before:transition-all before:duration-500 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  class="h-full w-full"
                  src="/images/douyin_ewm.jpg"
                />
              </div>
            </li>
          </ul>
        </div>
      </div>

      <ul class="!z-0 flex flex-wrap !duration-1000" data-aos="fade-up">
        <li class="w-1/2 md:w-1/4 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="与你相遇，好幸运！"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/175454FF3398EA377A652214492_F15B370F_1C996.jpg"
                class="aspect-[1/0.8693] object-cover w-full h-full lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-21</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                与你相遇，好幸运！
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>

        <li class="w-1/2 md:w-1/4 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="猜猜这是哪？"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/1AAD47A83DB0B869EAEBB11EB7C_C1D7EC53_A775.jpg"
                class="aspect-[1/0.8693] h-full w-full object-cover lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-18</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                猜猜这是哪？
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>

        <li class="w-full md:w-1/2 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="深职大，3金2银！"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/DC263E6255361CF2D6976FAC664_2A877B94_7CD3.jpg"
                class="aspect-[1/0.4345] w-full h-full object-cover lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-18</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                深职大，3金2银！
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>

        <li class="w-full md:w-1/2 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="定了！开始报名！"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/939021757320719DB84B52495BA_BB4676DE_305E.jpg"
                class="aspect-[1/0.4345] h-full w-full object-cover lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-16</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                定了！开始报名！
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>

        <li class="w-1/2 md:w-1/4 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="紧急通知！这里有超多岗位在等你！"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/84620C8308D997CE9D7CDA65219_8EB821F0_4998.jpg"
                class="aspect-[1/0.8693] h-full w-full object-cover lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-15</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                紧急通知！这里有超多岗位在等你！
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>

        <li class="w-1/2 md:w-1/4 relative">
          <NuxtLink
            class="group"
            to="https://mp.weixin.qq.com/s/zpHDixzQLBuTggxKSg4lAg"
            title="像素风深职大，也太真实了吧！"
          >
            <div
              class="relative w-full overflow-hidden after:absolute after:bottom-0 after:left-0 after:right-0 after:top-0 after:bg-black/30 after:content-[''] lg:after:duration-500 lg:group-hover:after:opacity-0"
            >
              <NuxtImg
                format="webp"
                quality="80"
                src="/images/2BE0ECA585B65978251E2DA2A6B_273789F6_172B0.jpg"
                class="aspect-[1/0.8693] object-cover w-full h-full lg:object-cover lg:transition-transform lg:duration-1000 lg:ease-[cubic-bezier(0.25,0,0,1)] lg:group-hover:scale-[1.08] select-none"
              />
            </div>
            <div
              class="absolute bottom-0 left-0 right-0 top-0 flex flex-col items-start justify-end px-[29.75px] text-white duration-500 lg:group-hover:translate-x-[8px] lg:group-hover:opacity-0"
            >
              <span style="font-size: 13.6px; line-height: 13.6px"
                >2024-10-14</span
              >
              <h4
                class="mb-[11.9px] mt-[6.8px] line-clamp-1"
                style="font-size: 16.15px; line-height: 20.1875px"
              >
                像素风深职大，也太真实了吧！
              </h4>
              <div
                class="relative flex items-center pb-[17px] after:absolute after:bottom-0 after:h-[5.94792px] after:w-full after:bg-[rgb(85,212,56)] after:content-['']"
              >
                <NuxtImg
                  format="webp"
                  quality="80"
                  src="/images/list9_ico2.png"
                  class="ml-[-5.1px] mr-[3.4px] max-h-[28.896px] max-w-[28.896px] overflow-clip"
                />
                <span style="line-height: 11.9px; font-size: 11.9px">微信</span>
              </div>
            </div>
          </NuxtLink>
        </li>
      </ul>
    </div>
  </section>
</template>

<style>
.home4-bg {
  background-image: url(~/assets/images/home4_bg.jpg);
}

.aos-init[data-aos][data-aos].aos-animate {
  transform: unset;
}
</style>
